...
import * as dataFormat from 'pages/CRUD/Announcementtypes/table/AnnouncementtypesDataFormatters';

import actions from 'actions/announcementtypes/announcementtypesListActions';
...
const AnnouncementtypesTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    ...
    { label: 'Word Count', title: 'wordcount', number: 'true' },
    { label: 'Imagecount', title: 'imagecount', number: 'true' },
  ...
  const count = useSelector((store) => store.announcementtypes.list.count);
  const modalOpen = useSelector(
    (store) => store.announcementtypes.list.modalOpen,
  );
  const rows = useSelector((store) => store.announcementtypes.list.rows);
  const idToDelete = useSelector(
    (store) => store.announcementtypes.list.idToDelete,
  );
  ...
  return (
    <div>
      <Widget title={<h4>Announcementtypes</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/announcementtypes/new'>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/announcementtypes/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Announcementtypes'
                : 'http://localhost:8080/api-docs/#/Announcementtypes'
            }
          >
            API documentation for announcementtypes
          </LinkMaterial>
        </div>
      </Widget>
...
export default AnnouncementtypesTable;

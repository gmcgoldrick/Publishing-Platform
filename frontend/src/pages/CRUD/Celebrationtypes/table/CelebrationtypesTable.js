...
import * as dataFormat from 'pages/CRUD/Celebrationtypes/table/CelebrationtypesDataFormatters';

import actions from 'actions/celebrationtypes/celebrationtypesListActions';
...
const CelebrationtypesTable = () => {
  ...
  const count = useSelector((store) => store.celebrationtypes.list.count);
  const modalOpen = useSelector(
    (store) => store.celebrationtypes.list.modalOpen,
  );
  const rows = useSelector((store) => store.celebrationtypes.list.rows);
  const idToDelete = useSelector(
    (store) => store.celebrationtypes.list.idToDelete,
  );
  ...
  return (
    <div>
      <Widget title={<h4>Celebrationtypes</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/celebrationtypes/new'>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/celebrationtypes/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Celebrationtypes'
                : 'http://localhost:8080/api-docs/#/Celebrationtypes'
            }
          >
            API documentation for celebrationtypes
          </LinkMaterial>
        </div>
      </Widget>
...
export default CelebrationtypesTable;
